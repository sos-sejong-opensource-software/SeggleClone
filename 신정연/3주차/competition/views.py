...
class CompetitionDetailView(APIView):
    ...
    def put(self, request, competition_id):
        ...
        #######################################################
        # os.path.isfile()
        # 파일인지 확인하고 맞으면 "True"를, 디렉토리이면 "False" Return
        # import os.path 해야함

        # shutil
        # 파일 복사
        # shutil.copyfile("./파일이름","./파일이름")
        # shutil.copy2: 메타 정보(예: 파일 작성 날짜)도 복사
...
class CompetitionUserView(APIView):
    ...
    def post(self, request, competition_id):
        ...
        if CompetitionUser.objcts.filter(username = request.user).filter(competition_id = competition_id).count():
            return Response({"error":"username이 이미 존재합니다."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def get(self, request, competition_id):
        competition = get_competition(competition_id)
        user_list = CompetitionUser.objects.filter(competition_id = competition_id)
...
class CompetitionTaView(APIView):
    ...
    def post(self, request, competition_id):
        ...
        for data in datas:
            ...
            is_check_competition_user_ta = CompetitionUser.objects.filter(username = data['username']).filter(privilega = 1).count()
            if is_check_user == 0:
