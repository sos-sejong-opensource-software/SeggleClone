...
class SubmissionCompetitionCsvDownloadView(APIView):
    permission_classes = [IsSubCompDownloadableUser] #대회 조교, 대회 생성자, 제출한 사람
    ...
    def get(self, request, submission_id):
        ...
        BASE_DIR = os.path.normpath(os.path.abspath(__file__).split('submission')[0])
        csv_path = str(submission.csv.path).split('uploads')[1]
        filename = csv_path.split('\\')[-1]
        filename = urllib.parse.quote(filename.encode('utf-8'))
        filepath = os.path.normpath(BASE_DIR + '/uploads/' + csv_path)
        ####################################################################
...
class SubmissionCompetitionIpynbDownloadView(APIView):
    ...
    def get(self, request, submission_id):
        ...
        ###################################################################### 
        # 윈도우에서 실행시키는 거라면 아래를 주석해제 해주시고 실행해주세요.
        BASE_DIR = os.path.normpath(os.path.abspath(__file__).split('submission')[0]) #'C:\\Users\\sungy\\BE3'
        ipynb_path = str(submission.ipynb.path).split('uploads')[1] #'\\submission\\96a8d722a2e844a5ba07febad867eafb\\ML5-2_upload.ipynb'
        filename = ipynb_path.split('\\')[-1]
        filename = urllib.parse.quote(filename.encode('utf-8')) #'ML5-2_upload.ipynb'
        filepath = os.path.normpath(BASE_DIR + '/uploads/' + ipynb_path) #'C:\\Users\\sungy\\BE3\\uploads\\submission\\96a8d722a2e844a5ba07febad867eafb\\ML5-2_upload.ipynb'
        ####################################################################
        ...
        # BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        # BASE_DIR = BASE_DIR.replace("/submission", "")
        # ipynb_path = str(submission.ipynb.path).split('uploads/', 1)[1]
