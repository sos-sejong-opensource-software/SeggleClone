...
class SubmissionClassView(APIView, EvaluationMixin):
    ...
    # 05-16 class id 수업의 contests_id contest의 cp_id인 문제에 파일 제출
...
class SubmissionClassCheckView(APIView):
    # 05-17 유저의 submission 중 어떤 것을 리더보드에 보일 것인지
    permission_classes = [IsClassUser]
...
class SubmissionCompetitionIpynbDownloadView(APIView):
    ...
    def get(self, request, submission_id):
        ...
        # 윈도우에서 실행시키는 거라면 아래를 주석해제 해주시고 실행해주세요.
        BASE_DIR = os.path.normpath(os.path.abspath(__file__).split('submission')[0])
        ipynb_path = str(submission.ipynb.path).split('uploads')[1]
        filename = ipynb_path.split('\\')[-1]
        filename = urllib.parse.quote(filename.encode('utf-8'))
        filepath = os.path.normpath(BASE_DIR + '/uploads/' + ipynb_path)
        ####################################################################
