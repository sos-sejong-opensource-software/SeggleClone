...
class CompetitionDetailView(APIView):
    ...
    def put(self, request, competition_id):
        ...
        #######################################################
        # os.path.isfile()
        # path가 존재하는 일반 file이면 True 반환

        # shutil
        # 파일 및 디렉토리에 대한 고수준 연산 제공 (복사, 삭제, ...)

        # shutil.rmtree
        # directory tree 전체를 삭제
        #######################################################
...
class CompetitionUserView(APIView):
    ...
    def post(self, request, competition_id):
        ...
        if CompetitionUser.objects.filter(username=request.user).filter(competition_id=competition_id).count():
            return Response({"error": "이미 참가한 대회입니다."}, status=status.HTTP_400_BAD_REQUEST)
        ########################
    ...
    def post(self, request, competition_id):
        ...
        for data in datas:
            ...
            if is_check_competition_user_ta:
                ...
                continue
            is_check_competition_user = CompetitionUser.objects.filter(username=data['username'].filter(competition_id=competition_id).filter(privilege=0)).count()
            if is_check_competition_user:
